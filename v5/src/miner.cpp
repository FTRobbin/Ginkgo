...
void transform_ord(mstate &s, const unsigned char *message) {
	for (int u = 0; u < rewind_len; ++u) {
		int i = s.ord[u] * 2;
		compress(s.m_h, message + (i << 6));
		compress(s.m_h, message + ((i | 1) << 6));
	}
}
...
void rewind_state(mstate &s) {
	s.m_tot_len = bak_m_tot_len;
	s.m_len = bak_m_len;
	memcpy(s.m_block, bak_m_block, sizeof(m_block));
	memcpy(s.m_h, bak_m_h, sizeof(m_h));

	s.rem_len = bak_rem_len;
}

void update_cont(mstate &s) {
	transform_ord(s, shifted_message + ((block_nb - rewind_len * 2) << 6));
	s.rem_len = new_len % SHA224_256_BLOCK_SIZE;
	memcpy(s.m_block, &shifted_message[block_nb << 6], s.rem_len);
	s.m_len = s.rem_len;
	s.m_tot_len += (block_nb + 1) << 6;
}
...
void end(mstate &s) {
	...
	int i;
	block_nb = (1 + ((SHA224_256_BLOCK_SIZE - 9)
				< (s.m_len % SHA224_256_BLOCK_SIZE)));
	len_b = (s.m_tot_len + s.m_len) << 3;
	pm_len = block_nb << 6;
	memset(s.m_block + s.m_len, 0, pm_len - s.m_len);
	s.m_block[s.m_len] = 0x80;
	SHA2_UNPACK32(len_b, s.m_block + pm_len - 4);
	transform(s.m_block, block_nb);
	for (i = 0 ; i < 8; i++) {
		SHA2_UNPACK32(s.m_h[i], &(s.digest[i << 2]));
	}
}

int num_threads;
pthread_mutex_t ord_lock = PTHREAD_MUTEX_INITIALIZER;
bool mined_flag;

void *rewind_sha256_single(void *ptr) {
	mstate s;
	s.ord = ord;
	while (!mined_flag) {
		rewind_state(s);
		...
		if (flag) {
			pthread_mutex_lock(&ord_lock);
			bool ret = next_permutation(ord.begin(), ord.end());
			s.ord = ord;
			pthread_mutex_unlock(&ord_lock);
			if (!ret) {
				break;
			} else {
				++rewind_cnt;
...
void rewind_sha256(unsigned char *digest, const char *input, const int length) {
	...
	for (int i = 0; i < num_threads; ++i) {
		pthread_create(&threads[i], NULL, &rewind_sha256_single, (void*)digest);
	}
	for (int i = 0; i < num_threads; ++i) {
