...
int get_type_size(const message_type &mt) {
	switch (mt) {
		...
		case TRANSACTION: return 1 + 32 * 4;
		case CLOSE: return 1;
		case BLOCK: return 1 + 32 * 5 + 128 * numtxinblock;
...
void broadcast(const message &m) {
	for (int i = 0; i < (int)peer_socks.size(); ++i) {
		if (peer_socks[i] != -1) {
			if (send(peer_socks[i], m.c_str(), m.size(), 0) == -1) {
...
void reply(const message &m) {
	if (send(curr_sock, m.c_str(), m.size(), 0) == -1) {
